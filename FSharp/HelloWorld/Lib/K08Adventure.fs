module HelloWorld.Lib.K08Adventure
    ...
    let rec private damagePlayer golemAttack playerHp =
        ...
        printfn $"ゴーレムがあなたを攻撃しました！攻撃値：{golemAttack}"
        playerHp - golemAttack
    
    let rec private battleLoop golemLevel golem playerHp =
        match golem.Hp with
        | 0 ->
            printfn $"ゴーレムLv.{golemLevel + 1}を倒した！"
            Continue playerHp
        | _ ->
            printfn $"ゴーレムLv.{golemLevel + 1}残りHP：{golem.Hp}"
            printfn "武器を選択してください（１．攻撃　２．特技　３．魔法）＞"
    ...
    let private engageBattle playerHp =
        ...
        printfn $"ゴーレムLv.{golemLevel + 1}が現れた！"
